  ...
  try {
    ...
    try {
      ...
      if (!userId) {
        ...
        return NextResponse.json({ error: "Unauthorized", details: "Please sign in to create a gist" }, { status: 401 })
      }

      user = await currentUser()
      console.log("Current user:", { id: user?.id, firstName: user?.firstName })

      if (!user) {
        console.log("No user found")
        return NextResponse.json({ error: "Unauthorized", details: "User session not found" }, { status: 401 })
      }
    } catch (authError) {
      console.error("Authentication error:", authError)
      return NextResponse.json(
        { error: "Authentication failed", details: "Please sign in and try again" },
        { status: 401 },
      )
    }
          ...
          return NextResponse.json(
            {
              ...
              details: uploadError instanceof Error ? uploadError.message : "Failed to upload file",
            },
            { status: 500 },
          )
        }
